...
export async function mockAiChat(
  ...
  const imageNote = opts?.hasImages
    ? '\n\ncamera *演示模式：图片已收到；配置 `DOUBAO_API_KEY` + `DOUBAO_VISION_MODEL` 后可真正识图。*'
    : '';
  ...
  if (shouldGenReq) {
    ...
    return {
      content: `已为你在「${domain.name}」领域生成结构化需求文档 sparkle\n\n**需求已整理完成**，你可以：\n• 点击下方卡片查看完整详情\n• 发布到需求广场\n• 进入智能匹配查看推荐${imageNote}`,
      reqCard: req,
  ...
  if (lower.includes('你好') || lower.includes('hi') || lower.includes('hello')) {
    return {
      content: `嗨！欢迎来到「${domain.name}」频道\n\n告诉我你想做什么项目，我来帮你整理需求、匹配协作者。\n\n直接说就行，不用想太多。${imageNote}`,
    };
  }
