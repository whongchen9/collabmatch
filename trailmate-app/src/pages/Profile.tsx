...
import { ArrowLeft, LogOut, Plus, Trash2, Save, Camera, MapPin, Sparkles, Wrench, X, Settings, Phone } from 'lucide-react';
import { useStore } from '@/store';
import { usersApi, gamificationApi, teamRankApi } from '@/api';
...
const CULTIVATION_REALMS = [
  { key: 'qi', name: '练气期', minHikes: 0, minDist: 0, emoji: '💨' },
  { key: 'foundation', name: '筑基期', minHikes: 1, minDist: 5, emoji: '🪨' },
  { key: 'golden', name: '金丹期', minHikes: 3, minDist: 20, emoji: '🌟' },
  { key: 'nascent', name: '元婴期', minHikes: 5, minDist: 50, emoji: '🌱' },
  { key: 'spirit', name: '化神期', minHikes: 10, minDist: 120, emoji: '🌊' },
  { key: 'tribulation', name: '渡劫期', minHikes: 20, minDist: 300, emoji: '⚡' },
  { key: 'mahayana', name: '大乘期', minHikes: 40, minDist: 600, emoji: '☀️' },
  { key: 'immortal', name: '散仙', minHikes: 80, minDist: 1500, emoji: '👑' },
];

/** 修仙境界英文名 */
const CULTIVATION_REALMS_EN: Record<string, string> = {
  qi: 'Qi Refining',
  foundation: 'Foundation Building',
  golden: 'Golden Core',
  nascent: 'Nascent Soul',
  spirit: 'Spirit Transformation',
  tribulation: 'Tribulation',
  mahayana: 'Mahayana',
  immortal: 'Loose Immortal',
};

/** 徒步人格英文名 */
const PERSONALITY_NAMES_EN: Record<string, string> = {
  navigator: 'Navigator',
  enjoyer: 'Enjoyer',
...
function computeRealm(hikeCount: number, totalDistKm: number) {
  let realm = CULTIVATION_REALMS[0];
  let next = CULTIVATION_REALMS[1];
  for (let i = CULTIVATION_REALMS.length - 1; i >= 0; i--) {
    const r = CULTIVATION_REALMS[i];
    if (hikeCount >= r.minHikes && totalDistKm >= r.minDist) {
      realm = r;
      next = CULTIVATION_REALMS[i + 1] || r;
      break;
    }
  }
  const progressHikes = Math.min(100, Math.round((hikeCount - realm.minHikes) / Math.max(1, next.minHikes - realm.minHikes) * 100));
  const progressDist = Math.min(100, Math.round((totalDistKm - realm.minDist) / Math.max(1, next.minDist - realm.minDist) * 100));
  const stability = Math.round((progressHikes + progressDist) / 2);
  return { ...realm, stability, nextName: next === realm ? '' : next.name };
}
...
export default function Profile() {
  ...
  useEffect(() => {
    if (user) {
      ...
      setResources(user.resources?.map((r: { text: string } | string) => typeof r === 'string' ? r : r.text) || []);
  ...
  useEffect(() => {
    if (isGuest) return;
    usersApi.getStats().then(setStats).catch(() => { /* ignore */ });
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, []);

  useEffect(() => {
    if (isGuest) return;
    gamificationApi.me().then(setGamiData).catch(() => { /* ignore */ });
    teamRankApi.me().then(res => { if (res.bestRank) setBestTeamRank(res.bestRank.rank); }).catch(() => { /* ignore */ });
    // 从 user 对象读取人格
    ...
    if ((user as UserExt)?.personality) setMyPersonality(PERSONALITY_MAP[(user as UserExt).personality!] || null);
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, []);
  ...
  if (!user) {
    if (isLoggedIn) {
      return <div className="flex items-center justify-center h-screen bg-[#faf7f2] dark:bg-gray-950">
        <div className="text-center"><div className="w-8 h-8 border-2 border-green-500 border-t-transparent rounded-full animate-spin mx-auto mb-3" /><p className="text-gray-400 dark:text-gray-500 text-sm">{t('common.loading')}</p></div>
      </div>;
    }
    return <div className="flex items-center justify-center h-screen bg-[#faf7f2] dark:bg-gray-950"><p className="text-gray-400 dark:text-gray-500">{language === 'en' ? 'Please log in' : '请先登录'}</p></div>;
  }
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      showToast(err instanceof Error ? err.message : t('profile.saveFailed'));
    } finally {
      setSaving(false);
    }
  };
  ...
  const handleFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      showToast(err instanceof Error ? err.message : (language === 'en' ? 'Avatar upload failed' : '头像上传失败'));
    } finally {
      setUploading(false);
      // 清空 input 以便再次选择同一文件
    ...
    <div className="pb-24 min-h-screen bg-[#faf7f2] dark:bg-gray-950">
      <div className="px-5 pt-6 pb-4 bg-white dark:bg-gray-900 sticky top-0 z-10 shadow-sm dark:shadow-gray-900/50 flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <h1 className="text-xl font-bold text-gray-800 dark:text-gray-200">{t('profile.title')}</h1>
        </div>
        {editing ? (
          <button onClick={handleSave} disabled={saving || isGuest} className="flex items-center gap-1 text-green-600 text-sm font-medium disabled:opacity-50">
            <Save className="w-4 h-4" />{saving ? (language === 'en' ? 'Saving...' : '保存中...') : t('common.save')}
          </button>
        ) : (
          <button onClick={() => !isGuest && setEditing(true)} className={`text-sm font-medium ${isGuest ? 'text-gray-300 dark:text-gray-600 cursor-not-allowed' : 'text-green-600'}`}>
            {isGuest ? (language === 'en' ? 'Edit (login required)' : '编辑（需登录）') : t('common.edit')}
          </button>
        )}
      </div>
      ...
      {isGuest && (
        <div className="mx-5 mt-3 p-3 bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-800 rounded-xl flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <span className="text-xs text-amber-700 dark:text-amber-300 font-medium">{language === 'en' ? 'Guest mode · Data saved locally only' : '访客模式 · 数据仅保存在本地'}</span>
          </div>
      ...
      <div className="px-5 mt-4 space-y-4">
                ...
                <div className="flex gap-1.5 mt-2 flex-wrap">
                  {myPersonality && (
                    <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[10px] font-bold bg-purple-50 dark:bg-purple-900/20 text-purple-600 dark:text-purple-300 border border-purple-100 dark:border-purple-800">
                      <span className="text-xs">{myPersonality.emoji || '🦅'}</span>{language === 'en' ? (PERSONALITY_NAMES_EN[myPersonality.key] || myPersonality.name) : myPersonality.name}
                    </span>
                  )}
                  {bestTeamRank && (
                  ...
                  {gamiData?.realm && (() => {
                    const cultivation = computeRealm(stats?.hikeCount || user?.hikeCount || 0, stats?.totalDistance || user?.totalDistance || 0);
                    return (
                    <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[10px] font-bold bg-green-50 dark:bg-green-900/20 text-green-600 dark:text-green-300 border border-green-100 dark:border-green-800">
                      {cultivation.emoji} {language === 'en' ? (CULTIVATION_REALMS_EN[cultivation.key] || cultivation.name) : cultivation.name} · {cultivation.stability}%
                    </span>
                  )})()}
                </div>
              </div>
        ...
        {editing && (
          <div className="bg-white dark:bg-gray-900 rounded-2xl shadow-sm dark:shadow-gray-900/50 p-4 space-y-4">
            <div>
              <label className="text-sm font-medium text-gray-700 dark:text-gray-300 mb-2 block">{t('profile.city')}</label>
              <input type="text" value={form.city} onChange={e => setForm(p => ({ ...p, city: e.target.value }))}
                placeholder={language === 'en' ? 'e.g. Shenzhen' : '例如：深圳'} className="w-full px-4 py-3 bg-gray-50 dark:bg-gray-800/50 rounded-xl border border-gray-200 dark:border-gray-700 text-sm outline-none focus:border-green-500" />
            </div>
            <div>
              <label className="text-sm font-medium text-gray-700 dark:text-gray-300 mb-2 block">{t('profile.bio')}</label>
              <textarea
                value={form.bio}
        ...
        {!editing && user.bio && (
          <div className="bg-white dark:bg-gray-900 rounded-2xl shadow-sm dark:shadow-gray-900/50 p-4">
            <label className="text-sm font-medium text-gray-700 dark:text-gray-300 mb-1 block">{t('profile.bio')}</label>
          ...
          >
            <div className="flex items-center justify-between mb-3">
              <div className="flex items-center gap-2">
                ...
                <h3 className="font-bold text-gray-800 dark:text-gray-200 text-sm">{language === 'en' ? 'Hike Logs' : '徒步日志'}</h3>
              </div>
            ...
            <div className="flex items-center gap-4">
              <div className="flex-1 text-center py-1.5 bg-green-50 dark:bg-green-900/20 rounded-xl">
                ...
                <p className="text-[10px] text-gray-400 dark:text-gray-500 mt-0.5">{language === 'en' ? 'Count' : '次数'}</p>
              </div>
              <div className="flex-1 text-center py-1.5 bg-green-50 dark:bg-green-900/20 rounded-xl">
                <p className="text-xl font-black text-green-600">{displayStats.totalDistance || 0}<span className="text-xs font-bold">km</span></p>
                <p className="text-[10px] text-gray-400 dark:text-gray-500 mt-0.5">{t('profile.totalDistance')}</p>
              </div>
            </div>
          </button>
        </div>
        ...
        <div className="bg-white dark:bg-gray-900 rounded-2xl shadow-sm dark:shadow-gray-900/50 p-4">
          <div className="flex items-center justify-between mb-3">
            <div className="flex items-center gap-1.5">
              ...
              <h3 className="font-semibold text-gray-800 dark:text-gray-200 text-sm">{language === 'en' ? 'Resources & Gear' : '资源设备'}</h3>
            </div>
            {editing && (
              <button onClick={() => setShowAddResource(true)}
                className="text-green-600 text-xs font-medium flex items-center gap-1">
                <Plus className="w-3.5 h-3.5" />{language === 'en' ? 'Add' : '添加'}
              </button>
            )}
          </div>
          ...
          {editing && showAddResource && (
            <div className="mb-3 p-3 bg-gray-50 dark:bg-gray-800/50 rounded-xl space-y-2">
              <input type="text" value={newResText} onChange={e => setResText(e.target.value)}
                placeholder={language === 'en' ? 'e.g. I have a car, pro camera...' : '例如：我有车、高端相机...'} className="w-full px-3 py-2 bg-white dark:bg-gray-900 rounded-lg border border-gray-200 dark:border-gray-700 text-sm outline-none focus:border-green-500" />
              <div className="flex gap-2">
                <button onClick={() => { setShowAddResource(false); setResText(''); }}
                  className="flex-1 py-1.5 rounded-lg text-xs font-medium bg-gray-200 text-gray-600 dark:text-gray-400">{t('common.cancel')}</button>
                <button onClick={addResource} disabled={!newResText.trim()}
                  className="flex-1 py-1.5 rounded-lg text-xs font-medium bg-green-600 text-white disabled:opacity-50">{language === 'en' ? 'Add' : '添加'}</button>
              </div>
            </div>
          )}
          ...
          {resources.length > 0 ? (
            ...
            <p className="text-xs text-gray-400 dark:text-gray-500 text-center py-4">{language === 'en' ? 'No resources yet' : '暂无资源设备'}</p>
          )}
        </div>
        ...
        <div className="bg-white dark:bg-gray-900 rounded-2xl shadow-sm dark:shadow-gray-900/50 p-4">
          <div className="flex items-center justify-between mb-3">
            <div className="flex items-center gap-1.5">
              ...
              <h3 className="font-semibold text-gray-800 dark:text-gray-200 text-sm">{t('profile.emergencyContact')}</h3>
            </div>
            {editing && (
          ...
          {emergencyContacts.length > 0 ? (
                ...
                editing ? (
                  <div key={i} className="p-3 bg-gray-50 dark:bg-gray-800/50 rounded-xl space-y-2">
                    <div className="flex items-center gap-2">
                      <input type="text" value={contact.name}
                        ...
                        placeholder={language === 'en' ? 'Name' : '姓名'} className="flex-[2] px-3 py-2 bg-white dark:bg-gray-900 rounded-lg border border-gray-200 dark:border-gray-700 text-sm outline-none focus:border-green-500 min-w-0" />
                      <input type="tel" value={contact.phone}
                        onChange={e => updateEmergencyContact(i, 'phone', e.target.value)}
                        placeholder={language === 'en' ? 'Phone' : '电话'} className="flex-[3] px-3 py-2 bg-white dark:bg-gray-900 rounded-lg border border-gray-200 dark:border-gray-700 text-sm outline-none focus:border-green-500 min-w-0" />
                      <button onClick={() => removeEmergencyContact(i)}
                  ...
                  <div key={i} className="flex items-center gap-3 px-3 py-2 bg-gray-50 dark:bg-gray-800/50 rounded-xl">
                    ...
                    <div className="flex-1 min-w-0">
                      <p className="text-sm font-medium text-gray-800 dark:text-gray-200">{contact.name || (language === 'en' ? 'Unnamed' : '未命名')}</p>
                      <p className="text-xs text-gray-400 dark:text-gray-500">{contact.phone || (language === 'en' ? 'No phone' : '无电话')}</p>
                    </div>
                  </div>
                )
              ))}
            ...
            <p className="text-xs text-gray-400 dark:text-gray-500 text-center py-4">{language === 'en' ? 'No emergency contacts' : '暂无紧急联系人'}</p>
          )}
        </div>
        ...
        <button onClick={() => navigate('/settings')}
          ...
          <div className="flex-1 min-w-0">
            <h3 className="font-bold text-gray-800 dark:text-gray-200 text-sm">{language === 'en' ? 'Settings' : '设置'}</h3>
            <p className="text-xs text-gray-400 dark:text-gray-500 mt-0.5">{language === 'en' ? 'App settings & API config' : '应用设置与 API 配置'}</p>
          </div>
          <span className="text-gray-300 dark:text-gray-600 text-lg">›</span>
        </button>
        ...
        <button onClick={handleLogout}
          ...
          <LogOut className="w-4 h-4" />{language === 'en' ? 'Log Out' : '退出登录'}
        </button>
      </div>
    </div>
  );
